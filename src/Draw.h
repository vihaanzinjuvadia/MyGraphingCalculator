...
void draw_line_plot_simple(sf::RenderWindow &window,
    ...
    for (int i {}; i < plotting_data.first.size() - 1; i++) {
        ...
        auto line_height_delta = 0.5;
        auto line {sf::RectangleShape(sf::Vector2f(line_height + line_height_delta, line_width))};
...
void draw_line_plot_advanced(sf::RenderWindow &window,
    ...
    // checking if there are any points to plot or not
...
void draw_axes(sf::RenderWindow &window) {
    ...
    sf::Font font;
    if (!font.loadFromFile("/home/vihaan/CLionProjects/MyGraphingCalculator/src/Font/SCRIPTIN.ttf"))
    {
        // Error
    }
